#!/usr/bin/env python3
"""Batch submit v21 — round 25. First full-time sweep run across BOTH the
...
CANDIDATES_PATH = Path("/tmp/claude-0/-home-user-Agents/9fd85063-ee93-588d-a736-933ead644540/scratchpad/final_r25.json")
...
def resolve_tenant(slug, offer_slug):
    """Recruitee tenants sometimes redirect (e.g. adesso -> werkenbijadesso). A POST does
    not follow redirects, so resolve the real tenant on a GET first."""
    probe = f"https://{slug}.recruitee.com/api/offers/{offer_slug}"
    try:
        out = subprocess.run(["curl", "-sI", "--max-time", "15", probe],
                             capture_output=True, text=True).stdout
        m = re.search(r"^location:\s*https://([^.]+)\.recruitee\.com", out, re.I | re.M)
        if m:
...
def submit_candidate(c):
    c["slug_company"] = resolve_tenant(c["slug_company"], c["offer_slug"])
