...
Endpoint: POST /api/v1/jobs/{shortcode}/apply
Body: { candidate: { ... } }
"""
...
def upload_resume():
    ...
    # S3 presigned POST - content-type must be in form fields
    form_data = dict(fields)
    form_data["Content-Type"] = "application/pdf"

    files = {"file": ("Hisham Abboud CV.pdf", file_content, "application/pdf")}
    ...
    s3_resp = requests.post(post_url, data=form_data, files=files, headers=upload_headers)
    print(f"S3 upload status: {s3_resp.status_code}")
    if s3_resp.status_code not in [200, 201, 204]:
        print(f"S3 error: {s3_resp.text[:300]}")
        return None

    print(f"Resume uploaded successfully! Key: {file_key}")
    return file_key


def submit_application(resume_key=None):
    """Submit the full application using the correct endpoint."""
...
Eindhoven, Netherlands"""
    ...
    # The form data structure based on the API we found
    candidate_data = {
        "firstname": "Hisham",
        "lastname": "Abboud",
    ...
    if resume_key:
        candidate_data["resume"] = resume_key

    payload = {"candidate": candidate_data}

    # The correct endpoint from the JS bundle
    url = f"https://apply.workable.com/api/v1/jobs/{JOB_SHORTCODE}/apply"
    print(f"POST {url}")
...
if __name__ == "__main__":
    ...
    if not resume_key:
        print("WARNING: Could not upload resume, submitting without it...")
    ...
    if result.status_code in [200, 201]:
        ...
        print("Response:", result.text[:500])
