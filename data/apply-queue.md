# Manual apply queue

Everything below passed the same filters as the automated path but needs a human
browser (captcha-gated form, custom questions, or an answer only you can give).
Work top to bottom; after submitting one, log it:

    python3 scripts/outcomes.py log "<Company>" --outcome acknowledged  # or record in tracker

## Modderkolk — Software Engineer
- URL: https://modderkolk.recruitee.com/o/software-engineer-3
- Location: Wijchen | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Hoe ben je op de 'werkenbij' website van Modderkolk terecht gekomen?
    - [optional] Indien anders, licht hieronder toe.
    - [optional] Indien je bent aangedragen door een medewerker van Modderkolk; noteer hieronder zijn/haar naam.
    - [required] Hoe wil je dat we contact met je opnemen?
- Cover letter (tailored on mes, testing):

```
Dear Hiring Team at Modderkolk,

I would like to apply for the Software Engineer position in Wijchen.

What drew me to this role is the overlap with my current work: supporting Manufacturing Execution Systems for industrial clients, which means working on software that runs live production environments. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Modderkolk.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Itility B.V. — DevOps Engineer / Site Reliability Engineer (SRE)
- URL: https://itilitybv.recruitee.com/o/devops-engineer-site-reliability-engineer-sre
- Location: Eindhoven | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Ben je momenteel woonachtig in Nederland en heb je een geldige werkvergunning voor Nederla -> **No**
- NEEDS YOUR OWN ANSWER:
    - [required] Hoe vaardig ben je in het Nederlands?
- Cover letter (tailored on python, data, azure, api):

```
Dear Hiring Team at Itility B.V.,

I would like to apply for the DevOps Engineer / Site Reliability Engineer (SRE) position in Eindhoven.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Itility B.V..

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## ABF — Software Developer
- URL: https://abfgroup.recruitee.com/o/software-developer
- Location: Roosendaal | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Where did you come across this vacancy?
- Cover letter (tailored on dotnet, testing, azure, api):

```
Dear Hiring Team at ABF,

I would like to apply for the Software Developer position in Roosendaal.

What drew me to this role is the overlap with my current work: building and maintaining applications in .NET, C# and ASP.NET at Actemium. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; working with Azure and CI/CD pipelines in an agile setting; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to ABF.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Blue Green — Medior Mendix Developer
- URL: https://bluegreen.recruitee.com/o/mendix-medior-developer
- Location: Amsterdam | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Ben je vloeiend en professioneel in de Nederlandse taal, zowel schriftelijk als mondeling? -> **Yes**
- NEEDS YOUR OWN ANSWER:
    - [required] Hoeveel jaar ervaring met Mendix heb je?
    - [required] Hoeveel platform punten heb je?
    - [optional] Kun je eens in de 2 weken bij ons op kantoor werken?
- Cover letter (tailored on mes):

```
Dear Hiring Team at Blue Green,

I would like to apply for the Medior Mendix Developer position in Amsterdam.

What drew me to this role is the overlap with my current work: supporting Manufacturing Execution Systems for industrial clients, which means working on software that runs live production environments. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Blue Green.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Channable — Python Software Engineer - Product team
- URL: https://channable.recruitee.com/o/python-software-engineer-product-team-1
- Location: Utrecht | ATS: recruitee
- Pre-computed answers (profile-backed):
    - How did you find out about Channable? -> **LinkedIn**
    - I confirm my availability for at least 32 hours per week, with a minimum of 2 days spent i -> **No**
- NEEDS YOUR OWN ANSWER:
    - [required] Do you have EU citizenship or a valid work permit for the Netherlands? Or do you need sponsorship from us?
    - [optional] If you selected valid work permit, please specify what type
    - [required] Where do you live? (City)
    - [optional] Which one of our company values resonates the most with you and why?
    - [optional] Want to impress us even more? Take your chance to introduce yourself to us (max. 2 mins)!
- Cover letter (tailored on python, testing, ai, azure):

```
Dear Hiring Team at Channable,

I would like to apply for the Python Software Engineer - Product team position in Utrecht.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Channable.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## DeepHealth — Junior Software Engineer, Clinical AI
- URL: https://deephealth.recruitee.com/o/junior-software-engineer-clinical-ai
- Location: Rotterdam | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Will you now or in the future require sponsorship to legally work in the country in which  -> **No**
- NEEDS YOUR OWN ANSWER:
    - [required] Are you able to work onsite at our Netherlands office at least two days per week?
    - [required] This is a junior-level Software Engineer position intended for an early-career candidate. Are you specifically
    - [required] When would you be available to start a new position?
    - [required] Do you have hands-on programming experience through professional work, university coursework/projects, interns
- Cover letter (tailored on mes, python, testing, ai):

```
Dear Hiring Team at DeepHealth,

I would like to apply for the Junior Software Engineer, Clinical AI position in Rotterdam.

What drew me to this role is the overlap with my current work: supporting Manufacturing Execution Systems for industrial clients, which means working on software that runs live production environments. I also bring day-to-day Python development, including a Flask backend for my own AI platform; building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to DeepHealth.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## BV Enersee — Backend Software Developer (Energy, Time Series & Data Flows)
- URL: https://enersee.recruitee.com/o/backend-software-developer-energy-time-series-data-flows
- Location: Antwerp | ATS: recruitee
- Pre-computed answers (profile-backed):
    - If you like, you can also share your Linkedin profile, instead of a resume here. -> **https://linkedin.com/in/hisham-abboud**
    - Where are you located? -> **Eindhoven, Netherlands**
    - Do you speak fluent English and at least one of these other languages: German, French or D -> **yes**
- NEEDS YOUR OWN ANSWER:
    - [required] Please share a github link or something comparable - we are looking forward to seeing your work!
- Cover letter (tailored on python, testing, ai, data):

```
Dear Hiring Team at BV Enersee,

I would like to apply for the Backend Software Developer (Energy, Time Series & Data Flows) position in Antwerp.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven, close to the Belgian border, and am happy to work in Belgium. I am fluent in Dutch and English. I would be glad to discuss how I can contribute to BV Enersee.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Customs Support Group — Customs AI Integration & Automation Engineer (Spain)
- URL: https://customssupport.recruitee.com/o/customs-ai-integration-automation-engineer-spain-3
- Location: Puerto de Algeciras (Cádiz) | ATS: recruitee
- Pre-computed answers (profile-backed):
    - What is your salary expectation (gross per month)? -> **EUR 4800 per month (~EUR 58k/yr)**
- NEEDS YOUR OWN ANSWER:
    - [required] Are you eligible to work in the country the position is located? Do you have a valid work permit?
    - [required] What's Your period of notice?
- Cover letter (tailored on python, testing, ai, data):

```
Dear Hiring Team at Customs Support Group,

I would like to apply for the Customs AI Integration & Automation Engineer (Spain) position in Puerto de Algeciras (Cádiz).

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Customs Support Group.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## FutureWhiz — Medior AI Product Engineer (Full-Stack)
- URL: https://futurewhiz.recruitee.com/o/medior-ai-product-engineer-full-stack
- Location: Amsterdam | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Do you require Visa sponsorship? -> **No**
- NEEDS YOUR OWN ANSWER:
    - [required] Are you currently based in the Netherlands?
- Cover letter (tailored on python, testing, ai, azure):

```
Dear Hiring Team at FutureWhiz,

I would like to apply for the Medior AI Product Engineer (Full-Stack) position in Amsterdam.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to FutureWhiz.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Greenchoice — Energie Data Developer Consument
- URL: https://greenchoice.recruitee.com/o/energie-data-developer-consument
- Location: Rotterdam | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Wat maakt dat je solliciteert op deze functie bij Greenchoice?
    - [required] Beschrijf een moment waarop data leidde tot een andere beslissing dan vooraf werd verwacht. Wat was jouw rol?
    - [required] Beschrijf een situatie waarin je zelf actief mensen hebt opgezocht om een vraagstuk verder te brengen. Wat lev
    - [required] Als je morgen bij Greenchoice start, welk vraagstuk rondom klantgedrag of energieverbruik zou je als eerste wi
- Cover letter (tailored on python, data, api, js):

```
Dear Hiring Team at Greenchoice,

I would like to apply for the Energie Data Developer Consument position in Rotterdam.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; designing REST API connections and integrations between systems; full-stack work with JavaScript and React alongside the backend. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Greenchoice.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## EVENTIM Benelux — Site Reliability Engineer
- URL: https://eventim.recruitee.com/o/site-reliability-engineer
- Location: Groningen | ATS: recruitee
- Pre-computed answers (profile-backed):
    - What are your salary expectations for this role (Please provide a gross monthly salary exc -> **EUR 4800 per month (~EUR 58k/yr)**
- NEEDS YOUR OWN ANSWER:
    - [required] Are you eligible to work in the Netherlands?
    - [required] Are you currently based in (or near) Groningen, or open for relocation?
    - [required] Do you have at least 2 years of experience in a Site Reliability Engineer, DevOps, or Systems Engineering role
    - [required] Tell us about a production infrastructure issue you solved. How did you investigate it, fix it and prevent it 
    - [required] What is your first possible starting date?
- Cover letter (tailored on python, testing, azure, api):

```
Dear Hiring Team at EVENTIM Benelux,

I would like to apply for the Site Reliability Engineer position in Groningen.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; working with Azure and CI/CD pipelines in an agile setting; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to EVENTIM Benelux.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## itsme® — Data Engineer
- URL: https://itsme.recruitee.com/o/data-engineer
- Location: Brussels | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Do you need a work permit to work in Belgium? -> **No**
- NEEDS YOUR OWN ANSWER:
    - [required] Do you live in Belgium?
- Cover letter (tailored on python, testing, ai, data):

```
Dear Hiring Team at itsme®,

I would like to apply for the Data Engineer position in Brussels.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven, close to the Belgian border, and am happy to work in Belgium. I am fluent in Dutch and English. I would be glad to discuss how I can contribute to itsme®.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## LEGALFLY — Junior Product Engineer
- URL: https://legalfly.recruitee.com/o/junior-product-engineer
- Location: Ghent | ATS: recruitee
- Pre-computed answers (profile-backed):
    - How did you hear about this job? -> **LinkedIn**
    - Where are you currently based? -> **Eindhoven, Netherlands**
    - Which languages do you speak fluently? -> **Other**
    - What is your desired salary? -> **EUR 4800 per month (~EUR 58k/yr)**
    - Could you share your LinkedIn and/or GitHub profile? -> **https://linkedin.com/in/hisham-abboud**
- NEEDS YOUR OWN ANSWER:
    - [required] What is your relevant experience regarding this position?
- Cover letter (tailored on ai, api, js, sql):

```
Dear Hiring Team at LEGALFLY,

I would like to apply for the Junior Product Engineer position in Ghent.

What drew me to this role is the overlap with my current work: building CogitatAI, an AI support platform with sentiment analysis, as my own project. I also bring designing REST API connections and integrations between systems; full-stack work with JavaScript and React alongside the backend; database work and query/schema optimisation as part of MES integrations. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven, close to the Belgian border, and am happy to work in Belgium. I am fluent in Dutch and English. I would be glad to discuss how I can contribute to LEGALFLY.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## NVM — Software Engineer - brainbay
- URL: https://nvm.recruitee.com/o/software-engineer-brainbay
- Location: Utrecht | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Are you currently residing in the Netherlands?
    - [required] Do you have a valid work permit?
- Cover letter (tailored on dotnet, testing, ai, azure):

```
Dear Hiring Team at NVM,

I would like to apply for the Software Engineer - brainbay position in Utrecht.

What drew me to this role is the overlap with my current work: building and maintaining applications in .NET, C# and ASP.NET at Actemium. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to NVM.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Profitap — QA Engineer
- URL: https://profitap.recruitee.com/o/qa-engineer-3
- Location: Eindhoven | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Do you require visa sponsorship? -> **No**
    - What is your gross salary expectation (monthly)? -> **EUR 4800 per month (~EUR 58k/yr)**
- NEEDS YOUR OWN ANSWER:
    - [required] What would be a suitable startdate for you?
    - [required] Are you open to relocating to the Eindhoven Area? (The Netherlands)
- Cover letter (tailored on python, testing, data, azure):

```
Dear Hiring Team at Profitap,

I would like to apply for the QA Engineer position in Eindhoven.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Profitap.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Realworks — Android Developer
- URL: https://realworks.recruitee.com/o/android-developer-3
- Location: Amsterdam | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Are you currently residing in the Netherlands?
- Cover letter (tailored on api):

```
Dear Hiring Team at Realworks,

I would like to apply for the Android Developer position in Amsterdam.

What drew me to this role is the overlap with my current work: designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Realworks.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## RTL — Data Platform Engineer
- URL: https://rtl.recruitee.com/o/data-platform-engineer
- Location: Hilversum | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Beheers je de Nederlandse taal op een goed niveau, in zowel woord als geschrift? -> **Yes**
    - Graag ontvangen we jouw bruto maandsalarisindicatie o.b.v. 40 uur. Dit helpt ons om in te  -> **EUR 4800 per month (~EUR 58k/yr)**
    - RTL verwerkt jouw persoonsgegevens in overeenstemming met DPG  privacybeleid . -> **Yes**
- NEEDS YOUR OWN ANSWER:
    - [required] Heb je in het verleden al voor RTL of DPG Media gewerkt?
- Cover letter (tailored on python, testing, data, azure):

```
Dear Hiring Team at RTL,

I would like to apply for the Data Platform Engineer position in Hilversum.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to RTL.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Ploonk (Smartphonehoesjes.nl) — QA Engineer
- URL: https://smartphonehoesjesnl.recruitee.com/o/qa-engineer-1
- Location: Leusden | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Do you have a permission to work in the Netherlands?
- Cover letter (tailored on testing, ai, data, azure):

```
Dear Hiring Team at Ploonk (Smartphonehoesjes.nl),

I would like to apply for the QA Engineer position in Leusden.

What drew me to this role is the overlap with my current work: building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML. I also bring building CogitatAI, an AI support platform with sentiment analysis, as my own project; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Ploonk (Smartphonehoesjes.nl).

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Swisscom — DevOps Engineer IAM (Ping IDM)
- URL: https://swisscom.recruitee.com/o/devops-engineer-iam-ping-idm
- Location: Rotterdam | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Current Location -> **Eindhoven, Netherlands**
    - Salary Expectations (in €) (please give a range of annual salary) -> **EUR 4800 per month (~EUR 58k/yr)**
    - LinkedIn Profile -> **https://linkedin.com/in/hisham-abboud**
- NEEDS YOUR OWN ANSWER:
    - [required] Nationality
    - [required] Date of birth
    - [required] Gender
    - [required] How did you hear about the Swisscom DevOps Center?
- Cover letter (tailored on testing, data, azure, api):

```
Dear Hiring Team at Swisscom,

I would like to apply for the DevOps Engineer IAM (Ping IDM) position in Rotterdam.

What drew me to this role is the overlap with my current work: building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML. I also bring building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Swisscom.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Twipe — Freelance Backend Engineer
- URL: https://twipe.recruitee.com/o/freelance-backend-engineer
- Location: Leuven | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Why are you interested in this role at Twipe?
    - [required] At Twipe, we value a hybrid work policy to connect with our colleagues. Are you open to commute to our office 
    - [required] We are looking to build the future of news on the long term with you. Are you open to a permanent employee con
- Cover letter (tailored on dotnet, testing, data, azure):

```
Dear Hiring Team at Twipe,

I would like to apply for the Freelance Backend Engineer position in Leuven.

What drew me to this role is the overlap with my current work: building and maintaining applications in .NET, C# and ASP.NET at Actemium. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven, close to the Belgian border, and am happy to work in Belgium. I am fluent in Dutch and English. I would be glad to discuss how I can contribute to Twipe.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Twipe — Backend Engineer
- URL: https://twipe.recruitee.com/o/backend-engineer
- Location: Leuven | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Why are you interested in this role at Twipe?
    - [required] At Twipe, we value a hybrid work policy to connect with our colleagues. Are you open to commute to our office 
    - [required] We are looking to build the future of news on the long term with you. Are you open to a permanent employee con
- Cover letter (tailored on dotnet, testing, data, azure):

```
Dear Hiring Team at Twipe,

I would like to apply for the Backend Engineer position in Leuven.

What drew me to this role is the overlap with my current work: building and maintaining applications in .NET, C# and ASP.NET at Actemium. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven, close to the Belgian border, and am happy to work in Belgium. I am fluent in Dutch and English. I would be glad to discuss how I can contribute to Twipe.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## VolkerWessels Infra Digital — Data Engineer
- URL: https://vwml.recruitee.com/o/data-engineer-1
- Location: Vianen | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Ik heb de  privacyverklaring  van VolkerWessels gelezen en begrijp hoe mijn persoonsgegeve -> **Yes**
    - Ben je woonachtig in Nederland? -> **Yes**
- NEEDS YOUR OWN ANSWER:
    - [required] Hoe ben je op deze website terecht gekomen?
    - [optional] Indien je bent aangedragen door iemand die hier werkt, noteer hieronder zijn/haar naam.
    - [required] Ben je in staat om op professioneel niveau Nederlands te spreken?
- Cover letter (tailored on mes, python, data, azure):

```
Dear Hiring Team at VolkerWessels Infra Digital,

I would like to apply for the Data Engineer position in Vianen.

What drew me to this role is the overlap with my current work: supporting Manufacturing Execution Systems for industrial clients, which means working on software that runs live production environments. I also bring day-to-day Python development, including a Flask backend for my own AI platform; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to VolkerWessels Infra Digital.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## GradeMatch — Software Test Engineer
- URL: https://veynexembeddedsoftware.recruitee.com/o/software-test-engineer
- Location: Eindhoven | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Beheers je de Nederlandse taal én woon je binnen een reistijd van maximaal één uur van het -> **Yes**
- Cover letter (tailored on python, testing, azure, api):

```
Dear Hiring Team at GradeMatch,

I would like to apply for the Software Test Engineer position in Eindhoven.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; working with Azure and CI/CD pipelines in an agile setting; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to GradeMatch.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## GradeMatch — LINUX | C++ | QT Developer
- URL: https://veynexembeddedsoftware.recruitee.com/o/linux-c-qt-developer
- Location: Eindhoven | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Beheers je de Nederlandse taal én woon je binnen een reistijd van maximaal één uur van het -> **Yes**
- Cover letter (tailored on python, testing, azure):

```
Dear Hiring Team at GradeMatch,

I would like to apply for the LINUX | C++ | QT Developer position in Eindhoven.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to GradeMatch.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Xomnia — Medior Data Engineer
- URL: https://xomnia.recruitee.com/o/medior-data-engineer
- Location: Amsterdam | ATS: recruitee
- Pre-computed answers (profile-backed):
    - What is your salary expectation? -> **EUR 4800 per month (~EUR 58k/yr)**
    - What is your current notice period? -> **Other**
    - What would your preferred start date be? -> **1 month notice period**
- NEEDS YOUR OWN ANSWER:
    - [required] How would you describe your current living/working situation?
    - [required] What is your proficiency in Dutch?
    - [required] How many hours would you want to work?
    - [required] How did you learn about Xomnia?
    - [optional] What are your preferred pronouns?
- Cover letter (tailored on python, testing, ai, data):

```
Dear Hiring Team at Xomnia,

I would like to apply for the Medior Data Engineer position in Amsterdam.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Xomnia.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Yuma — AI Engineer
- URL: https://yuma.recruitee.com/o/ai-engineer-3
- Location: Amsterdam | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Are you eligible to work in the EU
- Cover letter (tailored on python, ai, azure, js):

```
Dear Hiring Team at Yuma,

I would like to apply for the AI Engineer position in Amsterdam.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building CogitatAI, an AI support platform with sentiment analysis, as my own project; working with Azure and CI/CD pipelines in an agile setting; full-stack work with JavaScript and React alongside the backend. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Yuma.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Xebia — Data Engineer - Databricks
- URL: https://xccelerated.recruitee.com/o/data-engineer-databricks
- Location: Amsterdam | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Do you currently reside within the EU?
- Cover letter (tailored on testing, ai, data, azure):

```
Dear Hiring Team at Xebia,

I would like to apply for the Data Engineer - Databricks position in Amsterdam.

What drew me to this role is the overlap with my current work: building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML. I also bring building CogitatAI, an AI support platform with sentiment analysis, as my own project; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Xebia.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Xebia — GEN AI Engineer
- URL: https://xccelerated.recruitee.com/o/gen-ai-engineer
- Location: Randstad | ATS: recruitee
- NEEDS YOUR OWN ANSWER:
    - [required] Are you currently located in the Netherland
    - [required] Do you have any experience with leading organizational transformation efforts either in a product or general c
    - [required] What is your current proficiency in the Dutch language?
- Cover letter (tailored on testing, ai, azure, api):

```
Dear Hiring Team at Xebia,

I would like to apply for the GEN AI Engineer position in Randstad.

What drew me to this role is the overlap with my current work: building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML. I also bring building CogitatAI, an AI support platform with sentiment analysis, as my own project; working with Azure and CI/CD pipelines in an agile setting; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Xebia.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## coinmerce — DevOps Engineer
- URL: https://coinmerce.jobs.personio.de/job/2531536
- Location: Schiphol-Rijk | ATS: personio
- Cover letter (tailored on testing, data, azure, api):

```
Dear Hiring Team at coinmerce,

I would like to apply for the DevOps Engineer position in Schiphol-Rijk.

What drew me to this role is the overlap with my current work: building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML. I also bring building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project; working with Azure and CI/CD pipelines in an agile setting; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to coinmerce.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## saltocloudworks — Back-End Engineer
- URL: https://saltocloudworks.jobs.personio.de/job/2645148
- Location: Salto CloudWorks Amsterdam | ATS: personio
- Cover letter (tailored on dotnet, testing, azure, api):

```
Dear Hiring Team at saltocloudworks,

I would like to apply for the Back-End Engineer position in Salto CloudWorks Amsterdam.

What drew me to this role is the overlap with my current work: building and maintaining applications in .NET, C# and ASP.NET at Actemium. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; working with Azure and CI/CD pipelines in an agile setting; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to saltocloudworks.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## saltocloudworks — Flutter Developer
- URL: https://saltocloudworks.jobs.personio.de/job/2725444
- Location: Salto CloudWorks Amsterdam | ATS: personio
- Cover letter (tailored on azure, api):

```
Dear Hiring Team at saltocloudworks,

I would like to apply for the Flutter Developer position in Salto CloudWorks Amsterdam.

What drew me to this role is the overlap with my current work: working with Azure and CI/CD pipelines in an agile setting. I also bring designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to saltocloudworks.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## ore-energy — Software Engineer – Edge Systems & Embedded Security
- URL: https://ore-energy.jobs.personio.de/job/2738995
- Location: Amsterdam | ATS: personio
- Cover letter (tailored on mes, python, testing, data):

```
Dear Hiring Team at ore-energy,

I would like to apply for the Software Engineer – Edge Systems & Embedded Security position in Amsterdam.

What drew me to this role is the overlap with my current work: supporting Manufacturing Execution Systems for industrial clients, which means working on software that runs live production environments. I also bring day-to-day Python development, including a Flask backend for my own AI platform; building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to ore-energy.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## silverflow — Junior Data Engineer
- URL: https://silverflow.jobs.personio.de/job/2742497
- Location: NL | ATS: personio
- Cover letter (tailored on python, testing, ai, data):

```
Dear Hiring Team at silverflow,

I would like to apply for the Junior Data Engineer position in NL.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to silverflow.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## twelve — QA Engineer
- URL: https://twelve.jobs.personio.de/job/2732771
- Location: Amsterdam (hybrid) | ATS: personio
- Cover letter (tailored on testing, azure, js):

```
Dear Hiring Team at twelve,

I would like to apply for the QA Engineer position in Amsterdam (hybrid).

What drew me to this role is the overlap with my current work: building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML. I also bring working with Azure and CI/CD pipelines in an agile setting; full-stack work with JavaScript and React alongside the backend. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to twelve.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## scalian-germany — Consultant Embedded Software (f/m/d)*
- URL: https://scalian-germany.jobs.personio.de/job/2665769
- Location: Hengelo - Niederlande | ATS: personio
- Cover letter (tailored on testing, azure, api):

```
Dear Hiring Team at scalian-germany,

I would like to apply for the Consultant Embedded Software (f/m/d)* position in Hengelo - Niederlande.

What drew me to this role is the overlap with my current work: building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML. I also bring working with Azure and CI/CD pipelines in an agile setting; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to scalian-germany.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Blue Green Solutions — Medior Mendix Developer
- URL: https://bluegreen.recruitee.com/o/mendix-medior-developer
- Location: Amsterdam | ATS: recruitee
- Pre-computed answers (profile-backed):
    - Ben je vloeiend en professioneel in de Nederlandse taal, zowel schriftelijk als mondeling? -> **Yes**
- NEEDS YOUR OWN ANSWER:
    - [required] Hoeveel jaar ervaring met Mendix heb je?
    - [required] Hoeveel platform punten heb je?
    - [optional] Kun je eens in de 2 weken bij ons op kantoor werken?
- Cover letter (tailored on mes):

```
Dear Hiring Team at Blue Green Solutions,

I would like to apply for the Medior Mendix Developer position in Amsterdam.

What drew me to this role is the overlap with my current work: supporting Manufacturing Execution Systems for industrial clients, which means working on software that runs live production environments. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Blue Green Solutions.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Ore Energy — Software Engineer – Edge Systems & Embedded Security
- URL: https://ore-energy.jobs.personio.de/job/2738995
- Location: Amsterdam | ATS: personio
- Cover letter (tailored on mes, python, testing, data):

```
Dear Hiring Team at Ore Energy,

I would like to apply for the Software Engineer – Edge Systems & Embedded Security position in Amsterdam.

What drew me to this role is the overlap with my current work: supporting Manufacturing Execution Systems for industrial clients, which means working on software that runs live production environments. I also bring day-to-day Python development, including a Flask backend for my own AI platform; building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building data pipelines, including a GDPR-compliant anonymisation pipeline for my graduation project. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Ore Energy.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```

## Fairphone — Junior QA Tester
- URL: https://fairphone.jobs.personio.de/job/2762504
- Location: Amsterdam | ATS: personio
- Cover letter (tailored on python, testing, ai, api):

```
Dear Hiring Team at Fairphone,

I would like to apply for the Junior QA Tester position in Amsterdam.

What drew me to this role is the overlap with my current work: day-to-day Python development, including a Flask backend for my own AI platform. I also bring building a Python test suite with Pytest and Locust for performance and regression testing during my internship at ASML; building CogitatAI, an AI support platform with sentiment analysis, as my own project; designing REST API connections and integrations between systems. I hold a BSc in Software Engineering from Fontys and currently work as a Software Service Engineer at Actemium (VINCI Energies).

I am based in Eindhoven and open to working anywhere in the Netherlands. I would be glad to discuss how I can contribute to Fairphone.

Best regards,
Hisham Abboud
+31 6 4841 2838
hiaham123@hotmail.com
```
